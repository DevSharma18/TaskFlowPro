...
- `geminiService.call(templateKey, context, schema)` — model `gemini-3.5-flash`, `responseMimeType: application/json`, `responseSchema` enforced, temperature 0.2, 10s timeout, 3 retries w/ backoff, 5-min cache keyed on hash(template+context).
