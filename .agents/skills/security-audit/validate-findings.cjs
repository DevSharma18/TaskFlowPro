...
function readFileWithinLimit(file) {
  if (process.platform === "win32") {
    ...
    if (stat.isSymbolicLink()) throw new SafeInputError("input must not be a symlink");
    ...
    try {
      ...
      let total = 0;
      while (true) {
        const chunk = Buffer.alloc(Math.min(65536, LIMITS.inputBytes - total + 1));
      ...
      try {
        return UTF8_DECODER.decode(Buffer.concat(chunks, total));
      } catch {
