      ...
      <div className="glass-panel w-full max-w-2xl border border-slate-200/90 dark:border-surface-border p-6 my-8 text-slate-900 dark:text-slate-100 relative rounded-sm shadow-2xl animate-modal-enter">
        ...
        <div className="flex items-center justify-between pb-3.5 border-b border-slate-200/80 dark:border-surface-border mb-5">
          <div className="flex items-center space-x-2">
            <span className="text-xs font-mono text-brand-primary bg-brand-primary/10 border border-brand-primary/30 px-2 py-0.5 rounded-sm">
              {isEditing ? `TF-${taskId?.slice(0, 6)}` : 'NEW TASK'}
            </span>
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">
              {isEditing ? 'Task Details & Dependencies' : 'Create New Task'}
            </h2>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="glass-input w-full px-3 py-2 text-sm text-slate-900 dark:text-slate-100 font-medium rounded-sm"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {isEditing && (
                <button
                  ...
                  className="flex items-center space-x-1 text-[11px] text-brand-primary hover:underline disabled:opacity-50"
                >
            ...
            <textarea
              ...
              className="glass-input w-full px-3 py-2 text-xs text-slate-800 dark:text-slate-200 leading-relaxed rounded-sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Status Column</label>
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value as TaskStatus)}
                className="glass-input w-full px-3 py-2 text-xs text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900 rounded-sm"
              >
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">Story Points</label>
                {isEditing && (
                  <button
                    type="button"
              ...
              <input
                ...
                className="glass-input w-full px-3 py-2 text-xs text-slate-900 dark:text-slate-100 font-mono rounded-sm"
          ...
          {/* Scheduling Dates */}
          <div className="p-3 bg-slate-50 dark:bg-surface-50 border border-slate-200/80 dark:border-surface-border space-y-3 rounded-sm">
            <div className="flex items-center justify-between text-xs font-medium text-slate-800 dark:text-slate-300">
              ...
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-mono">Propagation shifts automatically</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1">Start Date</label>
                <input
                  type="date"
                  value={startDate}
                  onChange={(e) => setStartDate(e.target.value)}
                  className="glass-input w-full px-2 py-1.5 text-xs text-slate-800 dark:text-slate-200 rounded-sm"
                />
              </div>
              <div>
                <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1">End Date</label>
                <input
                  type="date"
                  value={endDate}
              ...
              <div>
                <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1">Duration (Days)</label>
                <input
                  type="number"
                  min="1"
                  value={durationDays}
                  onChange={(e) => setDurationDays(e.target.value === '' ? '' : Number(e.target.value))}
                  className="glass-input w-full px-2 py-1.5 text-xs text-slate-800 dark:text-slate-200 font-mono rounded-sm"
                  placeholder="e.g. 3"
                />
              </div>
            </div>
          ...
          {/* Dependencies Section (Editing Mode) */}
          {isEditing && (
            <div className="p-3 bg-slate-50 dark:bg-surface-50 border border-slate-200/80 dark:border-surface-border space-y-3 rounded-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <LinkIcon className="w-3.5 h-3.5 text-brand-primary dark:text-brand-accent" />
                ...
                <button
                  ...
                  disabled={Boolean(aiLoading)}
                  className="flex items-center space-x-1 text-[11px] text-brand-primary dark:text-brand-accent hover:underline disabled:opacity-50"
                >
                  <Sparkles className="w-3 h-3" />
              ...
              {taskData?.dependency_status === 'blocked' && (
                <div className="p-2.5 bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 text-xs flex items-start justify-between rounded-sm">
                  <div className="flex items-start space-x-2">
                    <Lock className="w-4 h-4 mt-0.5 text-rose-600 dark:text-rose-400 shrink-0" />
                    <div>
                  ...
                  <button
                    ...
                    className="text-[11px] underline text-rose-700 dark:text-rose-200 hover:text-rose-900 dark:hover:text-white shrink-0 ml-2"
                  >
                    Explain Chain
                  </button>
                </div>
              ...
              {aiBlockerExplanation && (
                <div className="p-2.5 bg-brand-primary/10 border border-brand-primary/30 text-slate-800 dark:text-slate-200 text-xs rounded-sm">
                  <p className="font-semibold text-brand-primary dark:text-brand-accent mb-1 flex items-center">
              ...
              <div>
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto">
                  {taskData?.prerequisites && taskData.prerequisites.length > 0 ? (
                    taskData.prerequisites.map((p) => (
                      <div
                        key={p.dep_id}
                        className="flex items-center justify-between p-2 bg-white dark:bg-surface-100 border border-slate-200 dark:border-surface-border text-xs rounded-sm"
                      >
                        <div className="flex items-center space-x-2 truncate">
                          <span className="font-mono text-[10px] text-slate-500 dark:text-slate-400">TF-{p.id.slice(0, 4)}</span>
                          <span className="truncate text-slate-800 dark:text-slate-200">{p.title}</span>
                          <span
                            className={`text-[10px] px-1 py-0.2 border rounded-sm ${
                              p.status === 'done'
                ...
                <div className="flex items-center space-x-2 mt-2">
                  <select
                    ...
                    className="glass-input flex-1 px-2.5 py-1.5 text-xs text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900 rounded-sm"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-surface-200 dark:hover:bg-surface-300 text-slate-800 dark:text-slate-200 text-xs font-medium border border-slate-200 dark:border-surface-border disabled:opacity-40 rounded-sm"
                  >
                    Add Prerequisite
                  </button>
                </div>
          ...
          {/* Action Buttons */}
          <div className="flex items-center justify-between pt-3.5 border-t border-slate-200/80 dark:border-surface-border">
            {isEditing ? (
              <button
                type="button"
                onClick={handleDelete}
                className="flex items-center space-x-1.5 px-3 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/30 text-xs font-medium transition-colors rounded-sm"
              >
            ...
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-surface-100 dark:hover:bg-surface-200 text-slate-700 dark:text-slate-300 text-xs font-medium border border-slate-200 dark:border-surface-border rounded-sm"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="px-5 py-2 bg-brand-primary hover:bg-brand-primary/90 text-white text-xs font-medium shadow-md shadow-brand-primary/20 rounded-sm"
              >
